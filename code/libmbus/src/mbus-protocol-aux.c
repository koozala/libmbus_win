...
int mbus_fixed_normalize(int medium_unit, long medium_value, char** unit_out, double* value_out, char** quantity_out)
{
	...
	{
	case 0x00:
		*unit_out = _strdup("h,m,s"); /*  todo convert to unix time... */
		*quantity_out = _strdup("Time");
		break;
	case 0x01:
		*unit_out = _strdup("D,M,Y"); /*  todo convert to unix time... */
		*quantity_out = _strdup("Time");
		break;
	...
	default:
		for (i = 0; fixed_table[i].vif < 0xfff; ++i)
		{
			...
			{
				*unit_out = _strdup(fixed_table[i].unit);
				*value_out = ((double)(medium_value)) * fixed_table[i].exponent;
				*quantity_out = _strdup(fixed_table[i].quantity);
				return 0;
			}
		}

		*unit_out = _strdup("Unknown");
		*quantity_out = _strdup("Unknown");
		exponent = 0.0;
		*value_out = 0.0;
		return -1;
	}
	...
	if (record)
	{
		...
		case 0x06: /* 6 byte integer (48 bit) */
			...
			else  // normal integer
			{
				...
				*value_out_real = (double)value_out_long_long;
			}
			break;
		...
		case 0x09: /* 2 digit BCD (8 bit) */
			*value_out_real = (double)mbus_data_bcd_decode(record->data, 1);
			result = 0;
			break;

		case 0x0A: /* 4 digit BCD (16 bit) */
			*value_out_real = (double)mbus_data_bcd_decode(record->data, 2);
			result = 0;
			break;

		case 0x0B: /* 6 digit BCD (24 bit) */
			*value_out_real = (double)mbus_data_bcd_decode(record->data, 3);
			result = 0;
			break;

		case 0x0C: /* 8 digit BCD (32 bit) */
			*value_out_real = (double)mbus_data_bcd_decode(record->data, 4);
			result = 0;
			break;
		...
		case 0x0E: /* 12 digit BCD (40 bit) */
			*value_out_real = (double)mbus_data_bcd_decode(record->data, 6);
			result = 0;
			break;
...
mbus_vif_unit_normalize(int vif, double value, char** unit_out, double* value_out, char** quantity_out)
{
	...
	for (i = 0; vif_table[i].vif < 0xfff; ++i)
	{
		...
		{
			*unit_out = _strdup(vif_table[i].unit);
			*value_out = value * vif_table[i].exponent;
			*quantity_out = _strdup(vif_table[i].quantity);
			return 0;
		}
	}

	MBUS_ERROR("%s: Unknown VIF 0x%03X\n", __PRETTY_FUNCTION__, newVif);
	*unit_out = _strdup("Unknown (VIF=0x%.02X)");
	*quantity_out = _strdup("Unknown");
	exponent = 0.0;
	*value_out = 0.0;
	return -1;
...
mbus_vib_unit_normalize(mbus_value_information_block* vib, double value, char** unit_out, double* value_out, char** quantity_out)
{
	...
	if (vib->vif == 0xFD) /* first type of VIF extention: see table 8.4.4 a */
	{
			...
			(vib->vif == 0xFC))
		{
			// custom VIF
			*unit_out = _strdup("-");
			*quantity_out = _strdup(vib->custom_vif);
			*value_out = value;
		}
		else
		{
...
mbus_parse_fixed_record(char status_byte, char medium_unit, unsigned char* data)
{
	...
	record->function_medium = _strdup(mbus_data_fixed_function((int)status_byte));  /* stored / actual */
	...
	{
		value = (long)mbus_data_bcd_decode(data, 4);
	}
	else
	{
...
mbus_parse_variable_record(mbus_data_record* data)
{
		...
		(data->drh.dib.dif == MBUS_DIB_DIF_MORE_RECORDS_FOLLOW)) /* MBUS_DIB_DIF_VENDOR_SPECIFIC */
	{
		...
		{
			record->function_medium = _strdup("More records follow");
		}
		else
		{
			record->function_medium = _strdup("Manufacturer specific");
		}
	...
	{
		record->function_medium = _strdup(mbus_data_record_function(data));
...
mbus_context_tcp(const char* host, uint16_t port)
{
	...
	tcp_data->port = port;
	if ((tcp_data->host = _strdup(host)) == NULL)
	{
...
mbus_context_set_option(mbus_handle* handle, mbus_context_option option, long value)
{
	...
	case MBUS_OPTION_PURGE_FIRST_FRAME:
		...
		{
			handle->purge_first_frame = (char)value;
			return 0;
		}
		break;
	}
...
mbus_scan_2nd_address_range(mbus_handle* handle, int pos, char* addr_mask)
{
	...
	if ((mask = _strdup(addr_mask)) == NULL)
	{
